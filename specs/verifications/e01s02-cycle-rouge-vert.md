...
| Le refus de mettre en pause un changement arrêté par une erreur d'exécution non levable nomme `resume` et `cancel`, et la reprise lève l'arrêt (`harness.test.ts`) | vert dès `54ce26f` : le comportement tenait déjà, le test le fixe ; il échoue quand `resumeLiftsStop` ne garde que `stop_retryable` | `54ce26f` |
