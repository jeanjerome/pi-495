# Gabarit de story — copie épinglée

Document écrit et tenu hors de 495. Ce dépôt ne le modifie pas : il en garde une copie parce qu'un
contrôle de Preflight en lit les noms de section, et qu'une adresse distante ne refuse rien.

| | |
|---|---|
| Amont | `docs/countable-story-format.md` du dépôt `danielvm-git/bigpowers` |
| Version | commit `9253dc0`, 7 juillet 2026 |
| Reprise | 21 septembre 2026 |
| Lu par | `scripts/check-story-format.ts` |

Le paquet bigpowers installé sous `.claude/skills/` ne distribue que les `SKILL.md` : l'arborescence
`docs/` de l'amont en est absente, alors que `slice-tasks` et `plan-work` y renvoient l'une et
l'autre. Sans cette copie, le format que les deux compétences exigent n'existe nulle part sur la
machine. Une resynchronisation se fait en comparant l'amont au texte qui suit la règle horizontale.

Tout ce qui suit est le texte amont, inchangé.
