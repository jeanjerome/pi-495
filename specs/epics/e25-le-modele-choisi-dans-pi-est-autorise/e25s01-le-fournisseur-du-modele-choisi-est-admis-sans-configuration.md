...
arrête tout changement jusqu'à ce qu'il soit réparé ou retiré et que Pi recharge ses extensions
(`/reload`) ou ouvre une nouvelle session. Un lien symbolique dont la cible a disparu est un fichier
qui ne s'ouvre pas, pas un fichier absent ; ce qui n'est pas un fichier ordinaire, un tube nommé
par exemple, est refusé avant d'être ouvert. Le refus tient toute la session parce que seule l'ouverture de session lie la
...
Scenario: Un fichier illisible arrête tout changement au lieu de céder aux défauts (6b)
  ...
  And   le fichier réparé n'est lu qu'au rechargement de Pi ou à l'ouverture d'une nouvelle session
...
- Fichiers touchés : `src/domain/policy.ts`, `src/extension/config.ts`,
  ...
  `test/v2/imposed-layers-divergence.test.ts`, `test/v2/harness.test.ts`, `test/v2/measure-budgets.test.ts`,
