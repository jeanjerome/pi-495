...
describe("component version change and dependent qualifications (EXT-02, REC-16, RM-076)", () => {
	...
	const probes = (version: string): Record<string, [string, string[]]> => ({ toolchain: [process.execPath, ["-e", `console.log("toolchain ${version}")`]] });
	...
	it("a locked component moving to another version is another environment", () => {
		...
		assert.deepEqual({ ...after.facts, tools: {} }, { ...before.facts, tools: {} }, "nothing but the component version differs");
	...
	it("a qualification established before the update is not reused after it, and the protocol carrying it no longer passes G2", () => {
		...
		const established = protocol({ environment_digest: before.ref.digest, qualifications: { unit: { positive: "PASS", negative: "FAIL", incident: "INDETERMINATE", qualified: true, environment_digest: before.ref.digest, notes: [] } } });
		assert.ok(reusableQualification([established], unit, before.ref.digest), "the same sensor in the same environment is not requalified");
		assert.equal(reusableQualification([established], unit, after.ref.digest), null, "the three witnesses have not answered under the new component version");
		...
		r.run({ type: "gate.evaluate", gate: "G2", at: tick(), actor: KERNEL, protocol_ref: ref("prt_1", established), protocol: established });
		assert.equal(r.s.gates.G2?.verdict, "FAIL");
		assert.ok(r.s.gates.G2?.reasons.some((reason) => reason.includes("environment")), r.s.gates.G2?.reasons.join("; "));
	});
	...
	it("the update invalidates the evidence and the gates that depended on the old environment, and sends the change back to the protocol", () => {
		...
		assert.ok(r.s.evidence.every((e) => !e.valid), "evidence measured in the former environment no longer counts");
		assert.deepEqual((["G2", "G3", "G4", "G5"] as const).filter((g) => r.s.gates[g]), [], "every gate from G2 onwards is invalidated");
		...
		running.run({ type: "intervention.start", at: tick(), actor: KERNEL, intervention_id: "int_x", role: "implement", attempt_id: "att_x", model: { provider_id: "p", model_id: "m", thinking_level: "off" }, profile_id: "implement", profile_qualified: true });
		running.expectError({ type: "environment.change", at: tick(), actor: KERNEL, digest: after.ref.digest }, "PRECONDITION_FAILED");
	});
});
