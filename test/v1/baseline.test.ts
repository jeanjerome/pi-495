...
function invocation(workspacePath: string, subjectDigest: string, kind: "reference" | "candidate"): ControlInvocation {
	return {
		control,
		protocol: { protocol_id: "prt_1", revision: 1, content_digest: digestValue("prt") },
		candidate: { candidate_id: kind, manifest_digest: subjectDigest, base_digest: subjectDigest, workspace_id: "ws" },
		subject: { kind, id: kind, revision: 1, digest: subjectDigest },
...
describe("the same control on the reference and on the candidate (VER-08)", () => {
	...
	async function passes(candidateSources: Record<string, string>, moveInWorkspace?: (path: string) => void) {
		...
		for (const [relative, content] of Object.entries(candidateSources))
			writeFileSync(join(candidateWorkspace.path, relative), content);
		...
		const run = runner();
		const onReference = (await run.runControl(invocation(referenceWorkspace.path, referenceDigest, "reference")))
			.evidence;
		const onCandidate = (await run.runControl(invocation(candidateWorkspace.path, candidateDigest, "candidate")))
			.evidence;
	...
	it("a tree carrying an inherited defect and an introduced one produces two distinct findings; only the introduced one blocks", async () => {
		...
		assert.deepEqual(
			onReference.findings.map((f) => f.path),
			["src/legacy.js"],
		);
		...
		assert.deepEqual(outcome.findings.map((f) => [f.path, f.baseline_state]).sort(), [
			["src/greet.js", "new"],
			["src/legacy.js", "preexisting"],
		]);
		assert.equal(outcome.verdict, "FAIL");
		assert.equal(
			outcome.comparison.blocking_findings,
			1,
			"the inherited defect stays visible and stops blocking; the introduced one blocks",
		);
	...
	it("a candidate that adds nothing to the inherited defect is not refused for it", async () => {
		...
		assert.equal(
			blockingCount(outcome.findings, "block_any"),
			1,
			"the other frozen tolerance would refuse the same candidate",
		);
	});

	it("renaming the file that carries the defect does not turn it into an introduced one", async () => {
		const { onReference, onCandidate, shape, reference } = await passes({}, (path) =>
			renameSync(join(path, "src", "legacy.js"), join(path, "src", "inherited.js")),
		);
		assert.deepEqual(
			onCandidate.findings.map((f) => f.path),
			["src/inherited.js"],
			"the control names the new path",
		);
		assert.notEqual(
			onCandidate.findings[0]?.fingerprint,
			onReference.findings[0]?.fingerprint,
			"and the finding is not the same one to a plain comparison",
		);
		...
		assert.deepEqual(
			outcome.findings.map((f) => f.baseline_state),
			["preexisting"],
		);
	...
	it("a control that answers differently on two passes of the same candidate keeps INDETERMINATE", async () => {
		...
		const confirmation = (await run.runControl(invocation(candidateWorkspace.path, candidateDigest, "candidate")))
			.evidence;
		...
		const outcome = applyInstability(
			compareToReference(
				...
				candidateShape(await workspace.snapshotCandidate(candidateWorkspace, reference, DEFAULT_WORKSPACE_POLICY)),
				"no_aggravation",
			),
