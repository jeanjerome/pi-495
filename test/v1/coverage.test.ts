...
import {
	COVERAGE_RULE_PARTIAL,
	COVERAGE_RULE_UNCOVERED,
	measurableIntroducedPaths,
	parseJacoco,
	...
	const body = sources
		.map((s) => {
			...
			const methods = (s.methods ?? [])
				.map(
					(m) =>
						`<method name="${m.name}" desc="()V" line="${m.line}"><counter type="INSTRUCTION" missed="0" covered="1"/></method>`,
				)
				.join("");
			const lines = s.lines
				.map((l) => `<line nr="${l.nr}" mi="${l.mi ?? 0}" ci="${l.ci ?? 0}" mb="${l.mb ?? 0}" cb="${l.cb ?? 0}"/>`)
				.join("");
...
describe("coverage of the introduced lines (QLT-04)", () => {
	it("an introduced line no test exercises fails, named at its file, at its line and at its symbol", () => {
		...
		const report = jacocoXml("io/h495", [
			{
				file: "Greeter.java",
				methods: [
					{ name: "&lt;init&gt;", line: 3 },
					{ name: "greet", line: 5 },
					{ name: "farewell", line: 9 },
				],
				lines: [
					{ nr: 3, mi: 3 },
					{ nr: 5, ci: 4 },
		...
		assert.deepEqual(
			[parsed.facts.measured_lines, parsed.facts.uncovered_lines, parsed.facts.tolerated_uncovered_lines],
			[3, 3, 0],
		);
		...
		assert.deepEqual(
			[findings[0]!.rule_id, findings[0]!.category, findings[0]!.severity],
			[COVERAGE_RULE_UNCOVERED, "quality", "blocker"],
		);
	});
	...
	it("a global ratio would have answered the other way round on both trees", () => {
		...
		assert.equal(
			parseJacoco(obs(), doc(poor), { [GREETER]: [5] }).verdict,
			"PASS",
			"a component historically below the line is not blocked for a candidate that adds nothing to its debt",
		);
		// 4 of 5 exercised: 80 %, above the same threshold, and the one line this candidate wrote is the missed one.
		const rich = jacocoXml("io/h495", [
			{
				file: "Greeter.java",
				methods: [{ name: "greet", line: 5 }],
				lines: [
		...
		assert.equal(
			parseJacoco(obs(), doc(rich), { [GREETER]: [22] }).verdict,
			"FAIL",
			"an untested addition is not compensated by the rest of the tree",
		);
	});

	it("an introduced line exercised on part of its branches is reported without blocking", () => {
		const report = jacocoXml("io/h495", [
			{ file: "Greeter.java", methods: [{ name: "greet", line: 5 }], lines: [{ nr: 5, ci: 4, cb: 1, mb: 1 }] },
		]);
	...
	it("an absent measurement is never read as coverage (QLT-02)", () => {
		const report = jacocoXml("io/h495", [
			{ file: "Greeter.java", methods: [{ name: "greet", line: 5 }], lines: [{ nr: 5, ci: 4 }] },
		]);
		...
		assert.equal(
			parseJacoco(obs({ exit_code: 3 }), doc(report), introduced).verdict,
			"INDETERMINATE",
			"the sensor itself failed",
		);
		assert.equal(
			parseJacoco(obs({ spawn_error: "ENOENT", exit_code: null }), doc(report), introduced).verdict,
			"INDETERMINATE",
			"broken runner",
		);
	...
	it("reads one report per module and refuses to guess an ambiguous attribution", () => {
		const shared = jacocoXml("io/h495", [
			{ file: "Adapter.java", methods: [{ name: "run", line: 3 }], lines: [{ nr: 3, mi: 2 }] },
		]);
		const perModule = [
			{ name: "domain/target/site/jacoco/jacoco.xml", text: shared },
			{ name: "infrastructure/target/site/jacoco/jacoco.xml", text: shared },
		];
		const parsed = parseJacoco(obs(), perModule, {
			"domain/src/main/java/io/h495/Adapter.java": [3],
			"infrastructure/src/main/java/io/h495/Adapter.java": [3],
		});
		assert.equal(parsed.verdict, "FAIL");
		...
		const ambiguous = parseJacoco(obs(), rootReport, {
			"domain/src/main/java/io/h495/Adapter.java": [3],
			"infrastructure/src/main/java/io/h495/Adapter.java": [3],
		});
...
describe("the coverage control through the generic runner", () => {
	...
	it("runs no measurement of its own, reads the report left in the workspace and locates each finding", async () => {
		...
		assert.ok(
			evidence.artifacts.some((a) => a.name === "report:target/site/jacoco/jacoco.xml"),
			"the measurement it judged is kept as evidence",
		);
		...
		assert.deepEqual(
			onReference.evidence.artifacts.filter((a) => a.name.startsWith("report:")),
			[],
		);
	...
	it("qualifies on a covered introduction, an unexercised introduction and a broken sensor (VER-05)", async () => {
		const covered = {
			file: "Witness495Covered.java",
			methods: [{ name: "twice", line: 3 }],
			lines: [
				{ nr: 1, ci: 3 },
				{ nr: 3, ci: 4 },
			],
		...
		const uncovered = {
			file: "Witness495Uncovered.java",
			methods: [{ name: "half", line: 3 }],
			lines: [
				{ nr: 1, mi: 3 },
				{ nr: 3, mi: 4 },
			],
		...
		const positiveFiles = {
			"src/main/java/Witness495Covered.java":
				"public final class Witness495Covered {\n    public int twice(int n) {\n        return n * 2;\n    }\n}\n",
		};
		const negativeFiles = {
			...positiveFiles,
			"src/main/java/Witness495Uncovered.java":
				"public final class Witness495Uncovered {\n    public int half(int n) {\n        return n / 2;\n    }\n}\n",
		};
		const runner = new GenericControlRunner(new UnconfinedSandbox(), new CasObjectStore(join(root, "objects")));
...
describe("the target adapter proposes the sensor only where a measurement exists", () => {
	it("adds the coverage control when JaCoCo writes a report at the test phase, with witnesses of its own", () => {
		...
		assert.deepEqual(
			[coverage!.parser, coverage!.report_path, coverage!.writable_paths],
			["jacoco-xml", "**/target/site/jacoco", []],
		);
		assert.ok(
			coverage!.protected_paths.includes("pom.xml"),
			"turning the measurement off in the POM is not the producer's to decide (QLT-04)",
		);
		...
		assert.deepEqual(Object.keys(detection.own_negative_witness.coverage ?? {}), [
			"src/main/java/witness495/Witness495Uncovered.java",
		]);
		assert.equal(detection.witness_tests, 2);
		assert.ok(
			!detection.capability_missing.some((note) => note.includes("JaCoCo")),
			"the measurement this sensor reads is the one mvn test writes",
		);
	});
	...
	it("names the missing measurement instead of proposing a sensor that would read nothing", () => {
		...
		assert.deepEqual(
			...
			["structure"],
			"no coverage witness where no coverage sensor is proposed",
		);
