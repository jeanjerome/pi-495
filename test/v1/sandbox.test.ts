...
describe("sandbox backends (SEC-01, SEC-02, ADR-013, C-SEC)", () => {
	darwinOnly(
		...
		async () => {
			...
			const obs = await sbx.run(profile(), {
	...
	darwinOnly("seatbelt lets a loopback profile reach itself and no other host (VER-04)", async () => {
		...
		const obs = await sbx.run(profile({ network: "loopback" }), {
			command: [NODE, "-e", script],
			cwd: join(root, "ws"),
			timeout_ms: 20000,
		...
		assert.equal(
			new TextDecoder().decode(obs.stdout).trim(),
			"self:ok,remote:EPERM",
	...
	it("a confinement tool that could not start reports an incident, not a verdict on the target (RM-016)", () => {
		...
		const refused = startupIncident(
			{ ...base, exit_code: 1, stderr: text("bwrap: Creating new namespace failed: Operation not permitted\n") },
			"bwrap: ",
			1,
			"fallback",
		);
		...
		assert.equal(
			startupIncident({ ...base, exit_code: 1, stderr: text("1 test failed\n") }, "bwrap: ", 1, "fallback"),
			null,
		);
		assert.equal(
			startupIncident({ ...base, exit_code: 0, stderr: text("bwrap: noise\n") }, "bwrap: ", 1, "fallback"),
			null,
		);
	});
	it("the process runner enforces timeout, kills the process group and bounds output (NFR-04, §12.3)", async () => {
		...
		const big = await sbx.run(profile(), {
		...
		const missing = await sbx.run(profile(), {
			command: ["/nonexistent/binary-495"],
			cwd: root,
			timeout_ms: 1000,
	...
	it("environment is built from the allowlist only, no shell is involved", async () => {
		...
		const obs = await sbx.run(profile(), {
			command: [
				NODE,
				"-e",
				"console.log(JSON.stringify({leak:process.env.HARNESS495_LEAK||null,test:process.env.HARNESS495_TEST,argv:process.argv.slice(1)}))",
				"$HOME",
				"a b",
			],
			cwd: root,
