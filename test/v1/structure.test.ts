...
const BOUNDARY: StructureRule = {
	rule_id: "structure:module-boundary:domain->infrastructure",
	kind: "forbidden_dependency",
	statement: "module demo-domain declares no dependency on module demo-infrastructure",
	scope: ["domain/src/main/java/"],
	forbidden: ["io.demo.infra"],
};
const NO_CYCLE: StructureRule = {
	rule_id: "structure:package-cycle",
	kind: "no_cycle",
	statement: "two packages that import each other are one unit",
	scope: ["domain/src/main/java/"],
	forbidden: [],
};
...
describe("reading what a Java tree declares about itself", () => {
	...
	it("attributes an imported name to the longest package the tree declares, which is the only way to place a static import", () => {
		...
		assert.equal(
			underPrefix("jakarta.persistence.Entity", "jakarta."),
			true,
			"a prefix ending in a dot names a family",
		);
...
describe("what the structural control blocks on (ARC-04, QLT-04)", () => {
	...
	it("an introduced import of a forbidden module fails, named at its file, at its line and at its package", () => {
		...
		assert.equal(
			finding.message,
			`${SERVICE}:4 forbidden import io.demo.infra.UserRepository, module demo-domain declares no dependency on module demo-infrastructure`,
		);
		assert.deepEqual(
			[finding.rule_id, finding.category, finding.severity, finding.symbol],
			[BOUNDARY.rule_id, "structure", "blocker", "io.demo.domain.service"],
		);
	...
	it("the same violation on a line the subject did not write is reported at its exact place and fails nothing", () => {
		...
		assert.equal(
			report.verdict,
			"PASS",
			"a control that answered otherwise could never be qualified on a target carrying any debt",
		);
		assert.equal(
			(report.findings ?? []).length,
			1,
			"the finding is what the comparison to the reference needs to pair the two passes",
		);
	...
	it("a cycle is named where the candidate closed it, and at its first import when nobody did", () => {
		...
		assert.equal(
			(closed.findings ?? [])[0]!.message,
			`${USER}:3 import io.demo.domain.service closes a dependency cycle between 2 packages (io.demo.domain.service, io.demo.domain.user), two packages that import each other are one unit`,
		);
		const inherited = analyzeJavaStructure(obs(), sources, [NO_CYCLE], {});
		assert.equal(inherited.verdict, "PASS");
		assert.equal(
			(inherited.findings ?? [])[0]!.message.startsWith(
				`${SERVICE}:3 import io.demo.domain.user closes a dependency cycle`,
			),
			true,
		);
	...
	it("an absent rule, an absent source, an unknown set of introduced lines and a broken sensor are all INDETERMINATE", () => {
		...
		assert.equal(
			analyzeJavaStructure(obs({ spawn_error: "ENOENT", exit_code: null }), sources, [BOUNDARY], {}).verdict,
			"INDETERMINATE",
		);
	});
});
...
describe("the boundaries a Maven reactor opposes to its own code (CON-03)", () => {
	it("derives them from the dependency direction of the POMs and the package root each module lays out", () => {
		...
		assert.equal(
			detection.controls.at(-1)!.control_id,
			"structure",
			"the test control stays the first of the protocol",
		);
		assert.deepEqual(
			control.structure_rules.map((rule) => rule.rule_id),
			[
				"structure:module-boundary:domain->infrastructure",
				"structure:framework-independence:domain",
				"structure:package-cycle",
			],
		);
		...
		assert.ok(
			control.structure_rules[1]!.forbidden.includes("org.springframework"),
			"the module the others build on carries no framework for them",
		);
		assert.deepEqual(control.structure_rules[2]!.scope, ["domain/src/main/java/", "infrastructure/src/main/java/"]);
		assert.ok(
			control.protected_paths.includes("domain/pom.xml"),
			"declaring the missing dependency is not the producer's way out of the boundary",
		);
		// Its own negative witness carries the defect a failing test could never exhibit.
		assert.deepEqual(Object.keys(detection.own_negative_witness.structure ?? {}), [
			"domain/src/main/java/witness495/Witness495Boundary.java",
		]);
	});
	...
	it("opposes no direction it cannot read, and says so instead of inventing a convention", () => {
		...
		assert.deepEqual(
			...
			["no_cycle"],
			"two modules laid out under the same package root cannot be told apart by an import",
		);
...
describe("the same sensor on the reference and on the candidate (ARC-04, VER-08)", () => {
	...
	it("a candidate placing a responsibility in a forbidden module is refused; the cycle it inherited is named and tolerated", async () => {
		...
		assert.deepEqual(
			onReference.findings.map((f) => `${f.path}:${f.region?.start_line}`),
			[`${SERVICE}:3`],
		);
		...
		assert.deepEqual(
			onCandidate.findings.map((f) => `${f.rule_id} ${f.path}:${f.region?.start_line}`),
			[`structure:module-boundary:domain->infrastructure ${SERVICE}:4`, `structure:package-cycle ${SERVICE}:3`],
		);

		const reference = {
			reference_id: "ref_1",
			reference_digest: digestValue("tree"),
			verdict: onReference.verdict,
			findings: onReference.findings.map((f): Finding => ({ ...f, baseline_state: "preexisting" })),
			evidence_id: "evr_1",
			reused: false,
		};
		const outcome = compareToReference(
			onCandidate.verdict,
			onCandidate.findings,
			reference,
			{ renames: new Map(), disappeared: new Set() },
			"no_aggravation",
		);
		assert.equal(outcome.verdict, "FAIL");
		assert.deepEqual(
			[outcome.comparison.new_findings, outcome.comparison.preexisting_findings, outcome.comparison.blocking_findings],
			[1, 1, 1],
		);
		assert.deepEqual(
			outcome.findings.map((f) => `${f.baseline_state} ${f.rule_id}`),
			["new structure:module-boundary:domain->infrastructure", "preexisting structure:package-cycle"],
		);
	});
	...
	it("a candidate that only inherits the cycle is accepted, and the cycle stays visible as preexisting", async () => {
		...
		const candidate = tree("candidate", {
			[USER]: CYCLIC_USER,
			[SERVICE]: FORBIDDEN_SERVICE.replace("import io.demo.infra.UserRepository;\n", ""),
		});
		...
		assert.deepEqual(
			outcome.findings.map((f) => `${f.baseline_state} ${f.rule_id}`),
			["preexisting structure:package-cycle"],
		);
	...
	it("is qualified on witnesses of its own: a boundary crossed, which no failing test would exhibit", async () => {
		...
		const q = await qualifyControl(
			runner,
			control,
			{ positive_path: positive, negative_path: negative, positive_files: {}, negative_files: negativeFiles },
			base(),
		);
		assert.deepEqual(
	...
	it("reads the declarations of the scopes the rules name, and of nothing else", async () => {
		...
		assert.deepEqual(
			read.notes,
			[],
			"a module without that source root is a fact of the tree, not a limit of the reading",
		);
	});
});
...
describe("what an intervention is told of its output contract (CTX-01, AGT-06)", () => {
	...
	it("shows a valid answer of the required shape, not a sketch of its types", () => {
		for (const [name, example] of Object.entries(OUTPUT_SCHEMA_EXAMPLES)) {
			assert.equal(
				Value.Check(OUTPUT_SCHEMAS[name as keyof typeof OUTPUT_SCHEMAS], example),
				true,
				`the example shown for ${name} does not satisfy the schema it illustrates`,
			);
		}
	...
	it("says what a missing block costs, since the kernel reads nothing else", () => {
		...
		assert.match(
			prompt,
			/discarded and the change stops/,
			"a model that does not know the fence is load-bearing has no reason to treat it as such",
		);
	});
	...
	it("does not tell a role that cannot write to leave the workspace building", () => {
		...
		for (const role of ["specify", "review", "observe"] as const) {
			assert.ok(
				!resumable.test(buildContext({ ...base, role }).system_prompt),
				`${role} writes nothing and is never resumed on a workspace`,
			);
		}
	...
	it("addresses the exact text it sent, so a dossier can be read back on it", () => {
		...
		assert.equal(
			built.manifest.prompt_digest,
			digestBytes(built.record),
			"the manifest addresses the record, not a reconstruction of it",
		);
...
describe("what the producer receives before it writes (ARC-04)", () => {
	...
	it("hands the frozen boundaries to the intervention that produces, and tells it they are read in its code", () => {
		...
		assert.match(
			prompt,
			/a control of the protocol reads in your code/,
			"an architecture announced without a control that observes it is a suggestion",
		);
		assert.ok(
			!buildContext({ ...input, role: "review" }).system_prompt.includes(BOUNDARY.statement),
			"a reviewer writes nothing to constrain",
		);
		assert.ok(
			!buildContext({ ...input, boundaries: [] }).system_prompt.includes("boundaries"),
			"a target that opposes no boundary is told of none",
		);
	});
});
