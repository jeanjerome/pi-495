...
describe("surviving mutants on the introduced lines (VER-04)", () => {
	it("a mutant surviving on a line the candidate wrote fails, named at its file, its line, its operator and its method", () => {
		...
		assert.equal(
			finding.message,
			`${GREETER}:5 introduced line whose mutation no test notices: Replaced integer multiplication with division (MathMutator) in io.h495.Greeter.twice`,
		);
		assert.deepEqual(
			[finding.rule_id, finding.category, finding.severity, finding.symbol],
			[MUTATION_RULE_SURVIVED, "quality", "blocker", "io.h495.Greeter.twice"],
		);
	});
	...
	it("a budget that ends the run gives INDETERMINATE, never FAIL, and the incident it names is what a retry may differ on", () => {
		const parsed = analyzeMutation(
			obs({ timed_out: true, exit_code: null, duration_ms: 1_800_000 }),
			null,
			{ [GREETER]: [5] },
			scopeOf(GREETER),
		);
	...
	it("reads one report per module and refuses to guess an ambiguous attribution", () => {
		const shared = pitestXml([
			{ status: "SURVIVED", line: 5, klass: "io.h495.Adapter", file: "Adapter.java", method: "run" },
		]);
		const perModule = [
			{ name: "domain/target/pit-reports/mutations.xml", text: shared },
			{ name: "infrastructure/target/pit-reports/mutations.xml", text: shared },
		];
		...
		const ambiguous = analyzeMutation(
			obs(),
			rootReport,
			{ [paths[0]!]: [5], [paths[1]!]: [5] },
			{ classes: ["io.h495.Adapter"], paths, notes: [] },
	...
	it("reads the report as the engine writes it, apostrophes and entities included", () => {
		...
		assert.equal(
			...
			`${GREETER}:7 introduced line whose mutation no test notices: replaced return value with "" for io/h495/Greeter::greet (EmptyObjectReturnValsMutator) in io.h495.Greeter.greet`,
		);
	});
...
describe("the mutation control through the generic runner", () => {
	...
	const ENGINE = [
		NODE,
		"-e",
		"const fs=require('node:fs');fs.mkdirSync('target/pit-reports',{recursive:true});fs.copyFileSync('495-mutations.xml','target/pit-reports/mutations.xml');process.stdout.write(process.argv.slice(1).join(' '));",
		"--",
	];
	...
	it("scopes the frozen command to the classes of the candidate, reads the report it left and locates each surviving mutant", async () => {
		...
		assert.ok(
			observed.artifacts.some((a) => a.name === "report:target/pit-reports/mutations.xml"),
	...
	it("spawns nothing on a subject that introduces no class, and nothing when no one established what it introduced", async () => {
		...
		const unscoped = await runner.runControl({
			...base(),
			control: unspawnable,
			workspace_path: ws,
			introduced_lines: null,
		});
	...
	it("a budget shorter than the run gives an incident, not a verdict on the candidate", async () => {
		...
		assert.equal(
			observed.limits.unstable,
			false,
			"the two marks a bounded technical retry reads: a named incident and no instability",
		);
	});

	it("qualifies on a killed introduction, an unasserted introduction and a broken engine (VER-05)", async () => {
		const covered =
			"public final class Witness495Covered {\n    public int twice(int n) {\n        return n * 2;\n    }\n}\n";
		const unasserted =
			"public final class Witness495Unasserted {\n    public int half(int n) {\n        return n / 2;\n    }\n}\n";
		const positiveFiles = { "src/main/java/Witness495Covered.java": covered };
		const negativeFiles = {
			...positiveFiles,
			"src/main/java/Witness495Unasserted.java": unasserted,
			"src/test/java/NegativeMutationWitness495Test.java": "public class NegativeMutationWitness495Test {}\n",
		};
		const killed = pitestXml([
		...
		const survivor = pitestXml([
			{ status: "KILLED", line: 3, klass: "Witness495Covered", file: "Witness495Covered.java" },
			{
				status: "SURVIVED",
				line: 3,
				klass: "Witness495Unasserted",
				file: "Witness495Unasserted.java",
				method: "half",
				description: "Replaced integer division with multiplication",
			},
		]);
		...
		const blind = await qualifyControl(
			runner,
			control({ command: ENGINE }),
			{ positive_path: pos, negative_path: pos, positive_files: positiveFiles, negative_files: positiveFiles },
			base(),
		);
...
describe("the target adapter proposes the sensor only where its report can be read", () => {
	it("adds the mutation control when the engine writes XML at a path no timestamp moves, with witnesses of its own", () => {
		...
		assert.deepEqual(
			[mutation!.parser, mutation!.report_path, mutation!.scope_argument],
			["pitest-xml", "**/target/pit-reports", "-DtargetClasses={classes}"],
		);
		assert.equal(mutation!.timeout_ms, 30 * 60_000, "an expensive control carries a budget of its own");
		assert.ok(
			mutation!.command.includes("-Dthreads=1"),
			"the run is single-threaded: a result that depends on scheduling is not reproducible",
		);
		assert.ok(
			mutation!.protected_paths.includes("pom.xml"),
			"lowering the engine's threshold or excluding a mutator is not the producer's to decide (VER-04)",
		);
		assert.equal(
			detection.controls.at(-1)!.control_id,
			"mutation",
			"the one control that runs a build of its own comes last",
		);
		...
		assert.deepEqual(Object.keys(detection.own_negative_witness.mutation ?? {}), [
			"src/main/java/witness495/Witness495Unasserted.java",
			"src/test/java/witness495/NegativeMutationWitness495Test.java",
		]);
	...
	it("names what the target would have to declare instead of proposing a sensor that would read nothing", () => {
		...
		assert.deepEqual(
			Object.keys(detection.own_negative_witness),
			["structure"],
			"no mutation witness where no mutation sensor is proposed",
		);
		...
		assert.equal(
			mutationCapabilityMissing({ declared: true, xml_report: false, stable_report_path: true, usable: false }),
			"a mutation engine is declared on this target but its report cannot be read: no XML report among its output formats (VER-04)",
		);
		assert.ok(
			mutationCapabilityMissing({
				...
				usable: false,
			}).includes("timestamped report directories"),
		);
	});
...
describe("what a mutation budget that runs out does to the change (VER-04, VER-02)", () => {
	...
	it("leaves the change indeterminate and opens an incident, never a correction charged to the candidate", () => {
		...
		assert.deepEqual(
			r.s.gates.G5?.fail_requirements,
			[],
			"no threshold is lowered and no defect is attributed to the candidate",
		);
