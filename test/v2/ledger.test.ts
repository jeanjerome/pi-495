...
describe("SQLite ledger: append-only chain and projections (EVD-02, EVD-03, NFR-03)", () => {
	it("appends events with a hash chain, projects the state and verifies integrity", async () => {
		...
		const second = ledger.appendChange("chg_1", 5, events.slice(5), {
			correlation_id: "cor_2",
			causation_id: first.event_ids[4] ?? null,
		});
	...
	it("refuses a concurrent write with REVISION_CONFLICT and never overwrites (RM-067)", () => {
		...
		assert.throws(
			() => ledger.appendChange("chg_1", 0, events.slice(0, 3), { correlation_id: "b" }),
			(e: Error) => /REVISION_CONFLICT|revision/.test(String((e as { code?: string }).code ?? e.message)),
		);
	...
	it("a crash before commit leaves the previous consistent state (fault injection)", async () => {
		...
		const ledger = new SqliteLedger(join(dir, "state.sqlite"), {
			hooks: {
				beforeCommit: () => {
					if (crash) throw new Error("simulated crash before COMMIT");
				},
			},
		});
		const events = sampleEvents();
		...
		crash = true;
		assert.throws(
			() => ledger.appendChange("chg_1", 4, events.slice(4, 9), { correlation_id: "b" }),
			/simulated crash/,
		);
		crash = false;
	...
	it("a crash between events and projection is atomic too, and survives reopening the file", async () => {
		...
		const ledger = new SqliteLedger(path, {
			hooks: {
				beforeProjection: () => {
					if (crash) throw new Error("crash before projection");
				},
			},
		});
		const events = sampleEvents();
	...
	it("detects a tampered event, a tampered projection and rebuilds the projection from events (F-EVIDENCE)", async () => {
		...
		ledger.db
			.prepare("UPDATE changes SET state = ? WHERE change_id = ?")
			.run(JSON.stringify({ ...ledger.loadChange("chg_1")!.state, outcome: "integrated" }), "chg_1");
		...
		ledger.db
			...
			.run(JSON.stringify({ ...target.event, at: "1999-01-01T00:00:00.000Z" }), target.event_id);
	...
	it("stores programs, artifacts, evidence, decisions, operations, bindings and leases", async () => {
			...
			[
				{
					...
					program_id: "prg_1",
					project_path: "/p",
					objective: { artifact_id: "obj", revision: 1, content_digest: digestValue("o"), schema_version: 1 },
					title: "T",
					budgets: { max_increments: 5, increments_started: 0, program_ms: 1000, program_ms_used: 0 },
				},
			],
			{ correlation_id: "x" },
		);
		...
		assert.equal(
			ledger.putArtifact("mandate", "chg_1", "mnd_1", obj, "kernel", "2026-09-16T10:00:01.000Z").revision,
			2,
		);
		...
		const evidence: Evidence = {
			...
			protocol_revision: { protocol_id: "p", revision: 1, content_digest: digestValue("p") },
			...
			facts: {},
			findings: [],
			artifacts: [{ name: "stdout", ref: obj }],
			limits: { truncated: false, bytes_read: 5, bytes_total: 5, exclusions: [], unstable: false, notes: [] },
			baseline: null,
			producer: KERNEL,
		...
		ledger.upsertOperation({
			operation_id: "op_1",
			idempotency_key: "k1",
			operation_type: "verification",
			aggregate_id: "chg_1",
			inputs_digest: digestValue(1),
			status: "accepted",
			effect_state: "none",
			result: null,
			created_at: "t",
		...
		assert.ok(
			ledger.acquireLease("change:chg_1", "owner-b", 60_000, "2026-09-16T10:02:00.000Z"),
	...
	it("refuses a database whose schema is newer (NFR-08)", () => {
		...
		const ledger = new SqliteLedger(path);
		ledger.db
			.prepare("INSERT INTO schema_migrations (version, digest, applied_at, result) VALUES (99, 'x', 't', 'applied')")
			.run();
		ledger.close();
...
describe("CAS object store (§7.3)", () => {
	...
	it("a crash before rename leaves no visible object, only a recoverable temporary", async () => {
		...
		const cas = new CasObjectStore(join(dir, "objects"), {
			beforeRename: () => {
				if (crash) throw new Error("crash before rename");
			},
		});
...
describe("Pi session lifecycle: reload, fork, and operations that must not run twice (UX-05, REC-40)", () => {
	...
	it("a reloaded extension and a forked conversation resolve the same change, and the control opened by the first session cannot be opened again by the second", () => {
		...
		assert.equal(
			reloaded.getSessionBinding("s1")?.change_id,
			"chg_1",
			"the binding lives in the ledger, not in the Pi session",
		);
		assert.equal(
			reloaded.loadChange("chg_1")?.state.operation?.operation_id,
			"op_v1",
			"the active operation is restored",
		);
		...
		assert.deepEqual(
			reloaded.findBindingsByCwd("/p").map((b) => b.change_id),
			["chg_1"],
			"it finds the change by its working directory",
		);
		...
		assert.throws(
			() =>
				reloaded.appendChange("chg_1", forked.revision, [opened("op_v2", key, "verification")], {
					correlation_id: "cor_s2",
				}),
	...
	it("an integration replayed under the same key runs once; a different key on an active operation is refused", () => {
		...
		assert.throws(
			() => ledger.appendChange("chg_1", at, [opened("op_i2", key, "integration")], { correlation_id: "c" }),
		...
		at = ledger.appendChange(
			...
			[
				...
				{ type: "operation.closed", at: tick(), actor: KERNEL, operation_id: "op_i1" },
			],
			{ correlation_id: "d" },
		).revision;
		assert.equal(ledger.getOperationByKey(key)?.status, "succeeded");
		assert.throws(
			() => ledger.appendChange("chg_1", at, [opened("op_i3", key, "integration")], { correlation_id: "e" }),
			/already holds the idempotency key/,
		);
	...
	it("the domain refuses a second operation on the same change before the ledger ever sees it (RM-067)", () => {
		...
		r.expectError(
			{ type: "verification.start", at: tick(), actor: KERNEL, operation_id: "op_v2", idempotency_key: "k2" },
			"OPERATION_ACTIVE",
		);
	});
});
