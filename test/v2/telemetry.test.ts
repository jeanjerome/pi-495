...
function instrumentNetwork(): { attempts: string[]; restore: () => void } {
	...
	const undo: (() => void)[] = [];
	const target = (arg: unknown): string => (typeof arg === "object" && arg !== null ? JSON.stringify(arg) : String(arg));
	...
	return { attempts, restore: () => { for (const u of undo.reverse()) u(); } };
}
...
describe("observability without imposed surveillance (NFR-06, REC-24)", () => {
	it("an instrumented change, from the request to the redacted export, opens no connection and resolves no host", async () => {
		...
		const t = track(makeHarness({
			...
			controls: (real) => ({ runControl: async (invocation, signal) => { profiles.push(`control:${invocation.control.control_id}:${invocation.control.network}`); return real.runControl(invocation, signal); } }),
		}));
		const startIntervention = t.agent.startIntervention.bind(t.agent);
		t.agent.startIntervention = async (mandate) => { profiles.push(`intervention:${mandate.role}:${mandate.profile.network}`); return startIntervention(mandate); };
		...
		try {
			...
			await exportChange(t.ledger, t.objects, { change_id: change.change_id, destination: dossier, redact: true, now: "2026-09-16T12:00:00.000Z", producer: "495 test" });
		} finally {
			restore();
		}
	...
	it("no source can reach a network: no client, no socket, no endpoint literal", () => {
		const outbound = /\bfetch\s*\(|\bhttps?\.(request|get)\s*\(|\bnet\.(connect|createConnection)\s*\(|\btls\.connect\s*\(|\bdgram\b|\bWebSocket\b|\bXMLHttpRequest\b|\bsendBeacon\b|from\s+["']node:(http|https|net|tls|dgram|dns)["']/;
