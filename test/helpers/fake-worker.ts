...
rl.on("line", (line) => {
	const msg = JSON.parse(line) as {
		type: string;
		mandate?: { objective: string; workspace_path: string };
		reason?: string;
	};
	if (msg.type === "abort") {
		aborted = true;
		...
		default:
			...
			send({
				...
				event: {
					...
					tool: "write",
					call_id: "c1",
					args_digest: `sha256:${"0".repeat(64)}`,
				},
			});
			send({
				type: "event",
				event: { type: "tool_finished", at: now(), tool: "write", call_id: "c1", is_error: false, blocked: false },
			});
			send({
				type: "event",
				event: {
