...
const loader = new DefaultResourceLoader({ cwd, agentDir: getAgentDir(), noExtensions: true, noSkills: true, noPromptTemplates: true, noThemes: true, noContextFiles: true, additionalExtensionPaths: [extension] });
await loader.reload();
const { session, extensionsResult } = await createAgentSession({ cwd, resourceLoader: loader, sessionManager: SessionManager.inMemory() });
...
session.subscribe((event) => {
	...
	if (message?.customType === "495") messages.push({ content: String(message.content ?? ""), details: message.details ?? {} });
});
