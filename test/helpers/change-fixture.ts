...
export const EXECUTOR: ActorRef = {
	actor_id: "executor",
	actor_type: "executor",
	role: "executor",
	origin: "executor",
		...
		requirements: [
			{
				...
				category: "functional",
				mandatory: true,
				criterion: "unit test greet('x') === 'Hello, x'",
				source: "request",
				contract_family: "api",
			...
			{
				requirement_id: "R2",
				statement: "no lint regression",
				category: "quality",
				mandatory: true,
				criterion: "lint exit code 0",
				source: "policy",
	...
	return {
		...
		controls: [
			...
			},
			{
				...
				network: "denied",
				writable_paths: [],
				requirement_refs: [{ requirement_id: "R2", revision: 1 }],
				protected: true,
				protected_paths: ["eslint.config.js"],
			},
		],
		qualifications: {
			unit: {
...
export function design(over: Partial<Design> = {}): Design {
	return {
		change_id: "chg_1",
		summary: "add greet in src/greet.ts",
		components: ["greet"],
...
export function candidate(seed: string, base = `sha256:${"0".repeat(64)}`): CandidateRef {
	return {
		candidate_id: `cand_${seed}`,
		manifest_digest: digestValue({ seed }),
		base_digest: base,
		workspace_id: "ws_1",
	};
}
...
export class Runner {
	...
	policy: ActivePolicy;
	constructor(
		policy: Partial<Omit<ActivePolicy, "budgets" | "adoption">> & {
		...
		this.policy = {
			...DEFAULT_POLICY,
			...policy,
			budgets: { ...DEFAULT_POLICY.budgets, ...(policy.budgets ?? {}) },
			adoption: { ...DEFAULT_POLICY.adoption, ...(policy.adoption ?? {}) },
		};
	}
	...
	create(over: Partial<Extract<ChangeCommand, { type: "change.create" }>> = {}): this {
		this.run({
			...
			program_id: "prg_1",
			increment_id: "inc_1",
			request: ref("req_1", { text: "Add greet" }),
			reference: { reference_id: "ref_1", kind: "git_clean_head", digest: digestValue({ tree: "base" }) },
			environment_digest: ENV,
			...over,
		});
		return this;
	...
	toDeciding(c: CandidateRef, verdicts: { unit?: EvidenceFact["verdict"]; lint?: EvidenceFact["verdict"] } = {}): this {
		...
		if (verdicts.unit !== undefined || Object.keys(verdicts).length === 0)
			facts.push(evidence({ control_id: "unit", subject_digest: c.manifest_digest, verdict: verdicts.unit ?? "PASS" }));
		if (verdicts.lint !== undefined || Object.keys(verdicts).length === 0)
