...
let root: string;
beforeEach(() => { mkdirSync(join(process.cwd(), "test-output"), { recursive: true }); root = mkdtempSync(join(process.cwd(), "test-output", "v3-")); });
...
function runPi(mode: "print" | "json", cwd: string, dataDir: string, prompt: string, extraEnv: Record<string, string> = {}): string {
	...
	const args = mode === "print" ? ["-ne", "-p", "--no-session", "-e", EXT, prompt] : ["-ne", "--mode", "json", "--no-session", "-e", EXT, prompt];
...
const SPEC = { objective: "tidy greet", facts: [], assumptions: [], questions: [], out_of_scope: [], risks: [], requirements: [{ requirement_id: "R1", statement: "greet unchanged", mandatory: true, criterion: "tests pass", category: "functional" }], design: { summary: "touch src/greet.js", components: [], interfaces: [], risks: [] } };
...
describe("Pi entries: print and JSON (C-PI)", { skip }, () => {
	it("the same start yields the same verdict in print and in JSON; JSON carries the canonical view (REC-39)", () => {
		...
		for (const mode of ["print", "json"] as const) {
			...
			const out = runPi(mode, proj, data, "/495 start tidy greet without behaviour change", { HARNESS495_SCRIPTED_AGENT: scriptFile(root, SPEC, RIGHT) });
			if (mode === "print") {
				assert.match(out, /Résultat: accepted/);
				assert.match(out, /G5=PASS/);
				outcomes.print = "accepted";
			} else {
				const lines = out.split("\n").filter(Boolean).map((l) => JSON.parse(l) as { type: string; message?: { customType?: string; details?: { view?: { change?: { outcome: string; gates: { gate: string; verdict: string }[] } } } } });
				const views = lines.filter((l) => l.type === "message_end" && l.message?.customType === "495" && l.message.details?.view?.change).map((l) => l.message!.details!.view!.change!);
	...
	it("a required human decision stops at a safe point with decision_required in print and JSON, without approval (SA-005, REC-21)", () => {
		...
		const out = runPi("print", proj, data, "/495 start tidy greet", { HARNESS495_SCRIPTED_AGENT: scriptFile(root, SPEC, RIGHT), HARNESS495_HUMAN_ACCEPTANCE: "1" });
