...
import {
	...
	normalizeOutput,
	retainedRefusedText,
} from "../../src/contracts/v1/reports.ts";
...
describe("structured output extraction (AGT-06)", () => {
	...
	it("normalizes a report with missing arrays and unknown keys without inventing content", () => {
		...
		const spec = normalizeOutput(OUTPUT_SCHEMAS["specification-report"], {
			objective: "o",
			requirements: [
				{ requirement_id: "R1", statement: "s", mandatory: true, criterion: "c", category: "f", note: "x" },
			],
			design: { summary: "d" },
		});
		...
		assert.equal(
			Value.Check(
				OUTPUT_SCHEMAS["specification-report"],
				normalizeOutput(OUTPUT_SCHEMAS["specification-report"], { facts: [] }),
			),
			false,
			"a missing objective stays invalid",
		);
	});
