...
import { applyProgram, decideProgram, eligibleIncrements, dependentsOf, findCycle, replayProgram, type IncrementSpec, type ProgramState } from "../../src/domain/program/program.ts";
...
function inc(id: string, depends_on: string[] = [], over: Partial<IncrementSpec> = {}): IncrementSpec {
	return { increment_id: id, title: id, kind: "functional", value: id, depends_on, required_capabilities: [], requirement_ids: [`REQ-${id}`], closure_criterion: "accepted", ...over };
}
...
class P {
	...
	run(c: Parameters<typeof decideProgram>[1]) {
		...
		for (const e of d.events) { this.state = applyProgram(this.state, e); this.events.push(e); }
		return this.state!;
	}
	...
	create() {
		this.run({ type: "program.create", at: tick(), actor: HUMAN, program_id: "prg_1", project_path: "/tmp/p", objective: ref("obj", "app"), title: "App" });
		return this;
	}
}
...
describe("program DAG (SA-006, SA-007, RM-006, RM-009)", () => {
	it("refuses a cyclic trajectory and locates the cycle", () => {
		...
		const d = decideProgram(p.state, { type: "trajectory.adopt", at: tick(), actor: HUMAN, increments: [inc("A", ["B"]), inc("B", ["C"]), inc("C", ["A"])], milestones: [], reason: "init" });
		assert.equal(d.ok, false);
		if (!d.ok) {
	...
	it("keeps independent increments eligible while a blocked increment blocks only its descendants", () => {
		const p = new P().create();
		p.run({ type: "trajectory.adopt", at: tick(), actor: HUMAN, increments: [inc("socle", [], { kind: "preparatory" }), inc("A", ["socle"]), inc("B", ["socle"]), inc("C", ["A"])], milestones: [], reason: "init" });
		assert.deepEqual(eligibleIncrements(p.state!).map((i) => i.increment_id), ["socle"]);
		p.run({ type: "increment.bind", at: tick(), actor: KERNEL, increment_id: "socle", change_id: "chg_s" });
		p.fail({ type: "increment.bind", at: tick(), actor: KERNEL, increment_id: "A", change_id: "chg_a" }, "PRECONDITION_FAILED");
		p.run({ type: "increment.result", at: tick(), actor: KERNEL, increment_id: "socle", status: "integrated", note: null });
		assert.deepEqual(eligibleIncrements(p.state!).map((i) => i.increment_id), ["A", "B"]);
		p.run({ type: "increment.bind", at: tick(), actor: KERNEL, increment_id: "A", change_id: "chg_a" });
		p.run({ type: "increment.result", at: tick(), actor: KERNEL, increment_id: "A", status: "blocked", note: "attempts_exhausted" });
		assert.deepEqual(eligibleIncrements(p.state!).map((i) => i.increment_id), ["B"]);
	...
	it("P0 runs one producing increment at a time", () => {
		const p = new P().create();
		p.run({ type: "trajectory.adopt", at: tick(), actor: HUMAN, increments: [inc("A"), inc("B")], milestones: [], reason: "init" });
		p.run({ type: "increment.bind", at: tick(), actor: KERNEL, increment_id: "A", change_id: "chg_a" });
		p.fail({ type: "increment.bind", at: tick(), actor: KERNEL, increment_id: "B", change_id: "chg_b" }, "OPERATION_ACTIVE");
	});
	it("an accepted increment cannot vanish from a revised trajectory; revision keeps statuses (PRG-04)", () => {
		const p = new P().create();
		p.run({ type: "trajectory.adopt", at: tick(), actor: HUMAN, increments: [inc("A"), inc("B", ["A"])], milestones: [], reason: "init" });
		p.run({ type: "increment.bind", at: tick(), actor: KERNEL, increment_id: "A", change_id: "chg_a" });
		p.run({ type: "increment.result", at: tick(), actor: KERNEL, increment_id: "A", status: "accepted", note: null });
		p.fail({ type: "trajectory.adopt", at: tick(), actor: HUMAN, increments: [inc("B")], milestones: [], reason: "drop A" }, "PRECONDITION_FAILED");
		p.run({ type: "trajectory.adopt", at: tick(), actor: HUMAN, increments: [inc("A"), inc("B", ["A"]), inc("D", ["A"])], milestones: [], reason: "add D" });
		assert.equal(p.state!.trajectory_revision, 2);
		assert.equal(p.state!.increments.find((i) => i.increment_id === "A")?.status, "accepted");
		assert.deepEqual(eligibleIncrements(p.state!).map((i) => i.increment_id), ["B", "D"]);
		assert.deepEqual(replayProgram(p.events), p.state);
	});
	it("an agent cannot write the program", () => {
		const p = new P().create();
		p.fail({ type: "trajectory.adopt", at: tick(), actor: { ...HUMAN, actor_type: "agent", origin: "model_output" }, increments: [inc("A")], milestones: [], reason: "x" }, "POLICY_DENIED");
	});
});
...
describe("milestones (SA-038, RM-007, RM-008, PRG-05)", () => {
	it("accepted increments with an unverified global requirement do not accept the program", () => {
		const p = new P().create();
		p.run({ type: "trajectory.adopt", at: tick(), actor: HUMAN, increments: [inc("A"), inc("B")], milestones: [{ milestone_id: "M1", title: "release", increment_ids: ["A", "B"], global_requirement_ids: ["PERF-1"], final: true }], reason: "init" });
		for (const id of ["A", "B"]) {
