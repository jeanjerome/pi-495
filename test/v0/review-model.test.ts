...
describe("line diff (UX-07, SA-023)", () => {
	it("keeps source lines exact, including literal + and - operators, and types segments", () => {
		...
		assert.deepEqual(segs.map((s) => s.kind), ["unchanged", "old", "new", "unchanged"]);
	...
	it("edit script reconstructs the new text for random inputs", () => {
		fc.assert(fc.property(fc.array(fc.constantFrom("a", "b", "c", "d"), { maxLength: 30 }), fc.array(fc.constantFrom("a", "b", "c", "d"), { maxLength: 30 }), (a, b) => {
			const ops = myers(a, b);
...
describe("review snapshot (SA-022, RM-058 to RM-061, SA-026, SA-028)", () => {
	const reference: ReferenceSnapshot = { reference_id: "ref_1", kind: "git_clean_head", project_path: "/p", head_commit: "a".repeat(40), branch: "main", tree_digest: digestValue("t"), entries: [entry("README.md", "unchanged"), entry("src/a.js", "unchanged"), entry("src/b.js", "unchanged"), entry("src/old.js", "unchanged", { content_digest: digestValue("same") }), entry("bin/x.bin", "unchanged")], exclusions: [], captured_at: "t", limits: { truncated: false, bytes_read: 0, bytes_total: 0, exclusions: [], unstable: false, notes: [] } };
	const manifest: CandidateManifest = { candidate_id: "cand_1", workspace_id: "ws", base_reference_id: "ref_1", base_digest: reference.tree_digest, selected_paths: [], exclusions: [], entries: [entry("README.md", "unchanged"), entry("src/a.js", "modified"), entry("src/b.js", "deleted"), entry("src/c.js", "added"), entry("src/old.js", "deleted", { content_digest: digestValue("same") }), entry("src/new.js", "added", { content_digest: digestValue("same") }), entry("bin/x.bin", "unchanged"), entry("link", "added", { kind: "symlink", symlink_target: "../etc" }), entry(`hostile${ESC}[31m.txt`, "added")], metadata_policy: "content_and_mode", manifest_digest: digestValue("m"), frozen_at: "t", limits: { truncated: false, bytes_read: 0, bytes_total: 0, exclusions: [], unstable: false, notes: [] } };
	it("builds the union of paths with textual statuses, keeps deleted files at their old place and detects a certain rename by digest", () => {
		const snap = buildSnapshot({ change_id: "chg", reference, manifest, findings: [], newer_candidate: null, now: "t" });
		...
		assert.ok(!all.some((x) => x.startsWith("src/old.js")), "the renamed source is presented once, at its new path with the old path attached");
	...
	it("signals a newer candidate without replacing the snapshot and neutralizes hostile names for display only", () => {
		const snap = buildSnapshot({ change_id: "chg", reference, manifest, findings: [], newer_candidate: "cand_2", now: "t" });
