...
| 2 | QLT-04 | couverture sur le code introduit, non-aggravation | livré pour une cible Maven dont JaCoCo lie `report` hors profil, mais le capteur ne se qualifie pas dans le cycle et G2 refuse alors le protocole (fiche D) ; la stack Node et les autres clauses de QLT-04 restent absentes | [02-qlt-04-couverture-introduite.md](02-qlt-04-couverture-introduite.md) |
...
| D | la qualification d'un capteur qui lit le rapport d'un autre contrôle, et le cycle réel qu'elle bloque | ouvert : sur une cible Maven, trois capteurs sur quatre se qualifient et `coverage` non, donc aucun protocole n'est gelé | [D-qualification-capteur-a-rapport.md](D-qualification-capteur-a-rapport.md) |
...
| F | une sortie structurée refusée, et le changement qui n'a plus d'issue | ouvert : le blocage est déclaré réessayable et aucune entrée Pi n'expose l'action nommée | [F-sortie-structuree-refusee.md](F-sortie-structuree-refusee.md) |
| G | la lisibilité de l'état d'un changement arrêté | ouvert : cause répétée trois fois, notification sur la ligne la moins décisive, huit risques résiduels sur douze issus des témoins de qualification | [G-lisibilite-etat-arrete.md](G-lisibilite-etat-arrete.md) |
