...
| D | la qualification d'un capteur qui lit le rapport d'un autre contrôle, et le cycle réel qu'elle bloquait | capteur clos : le rapport lu est déclaré par le contrôle qui l'écrit (`D-36`), les quatre capteurs se qualifient et la cible Maven va jusqu'à G5 sous agent déterministe ; reste le cycle avec le modèle, arrêté en `decision_required` sur `IH-01` | [D-qualification-capteur-a-rapport.md](D-qualification-capteur-a-rapport.md) |
...
| F | une sortie structurée refusée, et le changement qui n'a plus d'issue | ouvert, et c'est lui qui commande l'ordre : deux lancements sur trois y meurent sur la cible Maven, le blocage est déclaré réessayable et aucune entrée Pi n'expose l'action nommée | [F-sortie-structuree-refusee.md](F-sortie-structuree-refusee.md) |
