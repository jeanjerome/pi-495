		...
		if (!state.operation) {
			...
			if (diffEntries(referenceEntries, workingTree.entries).some((e) => e.baseline_state !== "unchanged")) {
				return h.commit(
					unit,
					{
						...
						detail:
							"the project tree differs from the reference captured at intake; re-verify or resolve before integrating (RM-054)",
					},
					cor,
				);
			}
			...
			return h.commit(
				unit,
				{
					...
					at: h.now(),
					actor: KERNEL_ACTOR,
					operation_id: h.id("op"),
					idempotency_key: `integrate:${manifest.manifest_digest}:${before}`,
					destination,
					destination_before: before,
		...
		if (state.operation.effect_state === "prepared") {
			...
			try {
				...
				if (info.is_repo) {
					...
					const r = await git(
						...
						[
							...
							"-q",
							"-m",
							`495: integrate candidate ${manifest.candidate_id}\n\nchange: ${state.change_id}\ncandidate: ${manifest.manifest_digest}`,
						],
						true,
					);
				...
				const appliedDigest = digestValue({
					...
					selected_paths: appliedEntries.filter((e) => e.baseline_state !== "unchanged").map((e) => e.path),
					exclusions: manifest.exclusions,
					entries: appliedEntries.map((e) => [
						e.path,
						e.kind,
						e.content_digest,
						e.mode,
				...
				return h.commit(
					unit,
					{
						...
						applied_digest: appliedDigest,
						receipt_digest: digestValue(receipt),
					},
					cor,
				);
			} catch (error) {
		...
		if (state.operation.effect_state === "started" || state.operation.effect_state === "uncertain") {
			return h.commit(
				unit,
				{
					...
					actor: KERNEL_ACTOR,
					reason: "integration_conflict",
					detail: "integration effect uncertain; reconcile (IH-12) before any retry",
				},
				cor,
			);
		}
		if (state.operation.effect_state === "reconciled") {
			...
			return h.commit(
				unit,
				{
					...
					applied: true,
					destination_after: after,
					receipt_digest: digestValue({ reconciled: after }),
				},
				cor,
			);
		}
