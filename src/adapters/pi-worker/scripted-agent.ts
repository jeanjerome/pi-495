...
export class ScriptedAgent implements AgentPort {
	...
	async describeCapabilities(model: ModelSelection): Promise<AgentCapabilities> {
		...
		return { provider_id: model.provider_id, model_id: model.model_id, available, reasons: available ? [] : ["scripted provider unavailable"] };
	}
	async startIntervention(mandate: InterventionMandate): Promise<InterventionHandle> {
		...
		const events: AsyncIterable<InterventionEvent> = {
			async *[Symbol.asyncIterator]() {
				...
				let toolCalls = 0;
				const counters = () => ({ tool_calls: toolCalls, duration_ms: Date.now() - started, tokens_known: script.tokens ?? 0, delegations: 0 });
				yield { type: "started", at: new Date().toISOString() };
				for (const step of script.steps) {
					if (aborted) { yield { type: "cancelled", at: new Date().toISOString(), counters: counters() }; return; }
					switch (step.kind) {
						case "write": {
							toolCalls++;
							const allowed = self.allowedTools(mandate).has("write");
							yield { type: "tool_started", at: new Date().toISOString(), tool: "write", call_id: `call_${toolCalls}`, args_digest: digestValue(step) };
							if (allowed && toolCalls <= mandate.budgets.tool_calls) {
								...
								yield { type: "tool_finished", at: new Date().toISOString(), tool: "write", call_id: `call_${toolCalls}`, is_error: false, blocked: false };
							} else yield { type: "tool_finished", at: new Date().toISOString(), tool: "write", call_id: `call_${toolCalls}`, is_error: true, blocked: true };
							break;
						}
						case "delete": {
							toolCalls++;
							yield { type: "tool_started", at: new Date().toISOString(), tool: "bash", call_id: `call_${toolCalls}`, args_digest: digestValue(step) };
							if (self.allowedTools(mandate).has("bash")) { await rm(join(mandate.workspace_path, step.path), { force: true }); yield { type: "tool_finished", at: new Date().toISOString(), tool: "bash", call_id: `call_${toolCalls}`, is_error: false, blocked: false }; }
							else yield { type: "tool_finished", at: new Date().toISOString(), tool: "bash", call_id: `call_${toolCalls}`, is_error: true, blocked: true };
							break;
						}
						case "tool":
							toolCalls++;
							yield { type: "tool_started", at: new Date().toISOString(), tool: step.tool, call_id: `call_${toolCalls}`, args_digest: digestValue(step) };
							yield { type: "tool_finished", at: new Date().toISOString(), tool: step.tool, call_id: `call_${toolCalls}`, is_error: step.is_error ?? false, blocked: step.blocked ?? !self.allowedTools(mandate).has(step.tool) };
							break;
						case "text":
				...
				yield { type: "completed", at: new Date().toISOString(), output: { raw: "" }, output_valid: false, counters: counters() };
			},
		};
		return { intervention_id: mandate.intervention_id, events, abort: async () => { aborted = true; } };
	}
