...
export class GenericControlRunner implements ControlExecutionPort {
	...
	async runControl(invocation: ControlInvocation, signal?: AbortSignal): Promise<{ evidence: EvidenceCandidate; observation: ProcessObservation | null }> {
		...
		try {
			...
			if (control.parser === "pitest-xml") {
				...
				if (lines === null) decided = unscopedMutation();
					...
					else if (control.scope_argument !== null) command = [...control.command, control.scope_argument.split(SCOPE_PLACEHOLDER).join(scope.classes.join(","))];
				}
			}
			if (decided) {
				report = decided;
			} else {
				observation = await this.sandbox.run(profile, { command, cwd, timeout_ms: control.timeout_ms, max_output_bytes: this.options.max_output_bytes }, signal);
				...
				switch (control.parser) {
					...
					case "pitest-xml": {
						...
						const docs = await readReports(invocation.workspace_path, control.report_path);
						for (const d of docs) artifacts.push({ name: `report:${d.name}`, ref: await this.objects.put(new TextEncoder().encode(d.text), "application/xml") });
						report = analyzeMutation(observation, docs, invocation.introduced_lines ?? null, scope!, `${stdoutText}\n${stderrText}`);
						break;
					}
					default:
		...
		const evidence: EvidenceCandidate = {
			...
			facts: { ...report.facts, command, cwd: control.cwd, sandbox: this.sandbox.backend, duration_ms: observation?.duration_ms ?? null },
			findings,
			artifacts,
