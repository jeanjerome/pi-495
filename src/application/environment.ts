...
function buildIdentity(): { version: string; build_digest: string } {
	...
	let version = "unknown";
	try {
		version = String((JSON.parse(readFileSync(join(root, "package.json"), "utf8")) as { version?: string }).version ?? "unknown");
	} catch {
...
export function describeEnvironment(piVersion: string, sandboxBackend: string, probes: Record<string, [string, string[]]> = { git: ["git", ["--version"]], java: ["java", ["-version"]], mvn: ["mvn", ["-v"]] }): { facts: EnvironmentFacts; ref: EnvironmentRef } {
	...
	const facts: EnvironmentFacts = { platform: process.platform, arch: process.arch, node: process.version, pi_version: piVersion, sandbox_backend: sandboxBackend, harness: harnessBuild(), tools };
