...
export function buildSnapshot(args: {
	...
	const entries: ManifestEntry[] =
		args.manifest?.entries ?? args.reference.entries.map((e) => ({ ...e, baseline_state: "unchanged" }));
	...
	const counts: Record<PathStatus, number> = {
		intact: 0,
		added: 0,
		modified: 0,
		deleted: 0,
	...
	for (const a of added) {
		const exact = deleted.find(
			(d) => d.content_digest && d.content_digest === a.content_digest && !consumed.has(d.path),
		);
		if (exact) {
			renamedFrom.set(a.path, { old: exact.path, certain: true });
			consumed.add(exact.path);
		}
	}
	for (const e of entries) {
		...
		if (rn) status = rn.certain ? "renamed" : "renamed?";
			...
			status =
				e.baseline_state === "unchanged"
					? "intact"
					: e.baseline_state === "added"
						? "added"
						: e.baseline_state === "deleted"
							? "deleted"
							: "modified";
		counts[status]++;
	...
	aggregate(root);
	return {
		snapshot_id: `rvw_${digestValue([args.reference.tree_digest, args.manifest?.manifest_digest ?? null]).slice(7, 19)}`,
		change_id: args.change_id,
		reference: {
			reference_id: args.reference.reference_id,
			kind: args.reference.kind,
			head_commit: args.reference.head_commit,
			tree_digest: args.reference.tree_digest,
		},
		candidate: args.manifest
			? {
					candidate_id: args.manifest.candidate_id,
					manifest_digest: args.manifest.manifest_digest,
					frozen_at: args.manifest.frozen_at,
				}
			: null,
		created_at: args.now,
		fresh: args.newer_candidate === null,
		newer_candidate: args.newer_candidate,
		complete: !(args.reference.limits.truncated || args.manifest?.limits.truncated),
		limits,
		root,
		counts,
...
function insert(root: ReviewNode, e: ManifestEntry, status: PathStatus, oldPath: string | null): void {
	...
	node.children.push({
		...
		kind: e.kind === "directory" ? "directory" : e.kind,
		status,
		old_path: oldPath,
		children: [],
		aggregate: {},
		limits: e.limits?.notes ?? [],
	});
}
...
function aggregate(node: ReviewNode): Partial<Record<PathStatus, number>> {
	...
	for (const c of node.children)
		for (const [k, v] of Object.entries(aggregate(c))) agg[k as PathStatus] = (agg[k as PathStatus] ?? 0) + (v ?? 0);
	node.aggregate = agg;
...
export function flatten(
	node: ReviewNode,
	changedOnly: boolean,
	out: { node: ReviewNode; depth: number }[] = [],
	depth = 0,
	expanded?: Set<string>,
): { node: ReviewNode; depth: number }[] {
	for (const c of node.children) {
		if (
			changedOnly &&
			c.status === "intact" &&
		...
		if (c.kind === "directory" && (!expanded || expanded.has(c.path)))
			flatten(c, changedOnly, out, depth + 1, expanded);
	}
	return out;
}
...
export async function readContent(
	...
	path: string,
	side: "old" | "new",
	page: { start_line: number; limit: number },
): Promise<ContentPage> {
...
export async function readChanges(
	...
	const notes: string[] = [];
	const kind: ChangePage["kind"] =
		oldSide.kind === "text" || newSide.kind === "text"
			? [oldSide.kind, newSide.kind].some((k) => k === "binary")
				? "binary"
				: [oldSide.kind, newSide.kind].some((k) => k === "too_large")
					? "too_large"
					: "text"
			: oldSide.kind === "missing"
				? newSide.kind
				: oldSide.kind;
	...
	if (kind !== "text")
		return {
			...
			intraline: {},
			metadata,
			notes: [`no textual comparison for ${kind} content`],
		};
	...
	for (const hk of h)
		for (let i = 0; i + 1 < hk.segments.length; i++) {
			...
			if (a.kind === "old" && b.kind === "new" && a.lines.length === b.lines.length)
