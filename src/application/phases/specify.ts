...
export async function specify(ctx: PhaseContext, unit: Unit, cor: string): Promise<Unit> {
	...
	const ref = await ctx.artifacts.store("requirements", unit.state.change_id, ctx.id("rqs"), doc, KERNEL_ACTOR.actor_id);
	unit = ctx.commit(unit, { type: "artifact.propose", at: ctx.now(), actor: KERNEL_ACTOR, kind: "requirements", ref }, cor);
	unit = ctx.commit(unit, { type: "gate.evaluate", gate: "G1", at: ctx.now(), actor: KERNEL_ACTOR, requirements_ref: ref, requirements: doc, report: { valid: issues.length === 0, issues } }, cor);
