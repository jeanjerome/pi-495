...
export function detectStack(projectPath: string, requirementRefs: RequirementRef[], nodeBinary = process.execPath): StackDetection {
	...
	if (existsSync(pomPath)) {
		...
		const controls: ControlDefinition[] = [
			{ control_id: "maven-test", version: "1", title: "mvn test (Surefire)", command: ["mvn", "-B", "-q", "-o", "test"], cwd: ".", env_allowlist: BASE_ENV, env: {}, timeout_ms: 20 * 60_000, parser: "junit-xml", report_path: "**/target/surefire-reports", structure_rules: [], provides: ["surefire-reports", ...(jacoco ? ["jacoco-report"] : [])], requires: [], scope_argument: null, network: "denied", writable_paths: reactor.target_paths, requirement_refs: requirementRefs, protected: true, protected_paths: [...reactor.preparation_paths, ...reactor.pom_paths] },
		];
		...
		if (rules.length > 0) controls.push({ control_id: "structure", version: "1", title: "frozen architecture boundaries, read from the Java declarations", command: [nodeBinary, "-e", ""], cwd: ".", env_allowlist: BASE_ENV, env: {}, timeout_ms: 120_000, parser: "java-imports", report_path: null, structure_rules: rules, provides: [], requires: [], scope_argument: null, network: "denied", writable_paths: [], requirement_refs: requirementRefs, protected: true, protected_paths: [...reactor.pom_paths] });
