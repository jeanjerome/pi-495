...
export function diagnoseControlCapability(input: CapabilityInput): ControlCapabilityDiagnosis {
	...
	if (discriminant)
		notes.push(`prepared suite adopted: ${input.prepared!.files.length} file(s) failing on the reference`);
	...
	return {
		...
		discovered,
		executed,
		undiscriminated_requirements: undiscriminated,
		unobserved_requirements: unobserved,
		notes,
	};
}
