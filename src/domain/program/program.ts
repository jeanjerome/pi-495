...
export type ProgramEvent =
	| (Base & { type: "program.created"; program_id: string; project_path: string; objective: ArtifactRef; title: string; budgets: ProgramState["budgets"] })
	| (Base & { type: "trajectory.adopted"; revision: number; increments: IncrementSpec[]; milestones: Milestone[]; reason: string })
...
export type ProgramCommand =
	| (Base & { type: "program.create"; program_id: string; project_path: string; objective: ArtifactRef; title: string; budgets?: Partial<ProgramState["budgets"]> })
	...
	| (Base & { type: "increment.result"; increment_id: string; status: "accepted" | "integrated" | "blocked" | "abandoned" | "active" | "planned"; note: string | null })
	| (Base & { type: "milestone.evaluate"; milestone_id: string; global_verdicts: Record<string, Verdict>; integrated_digest: string | null })
...
export function applyProgram(state: ProgramState | null, event: ProgramEvent): ProgramState {
	if (event.type === "program.created") {
		return { schema_version: 1, program_id: event.program_id, project_path: event.project_path, objective: event.objective, title: event.title, revision: 1, trajectory_revision: 0, increments: [], milestones: [], milestone_evaluations: [], budgets: event.budgets, closed: false, created_at: event.at, updated_at: event.at, last_actor: event.actor };
	}
	...
	switch (event.type) {
		case "trajectory.adopted": {
			...
			s.increments = event.increments.map((spec) => {
				...
				return { ...spec, status: old?.status ?? "planned", change_id: old?.change_id ?? null, result_note: old?.result_note ?? null };
			});
			s.milestones = event.milestones;
			return recomputeEligibility(s);
		}
		case "increment.bound":
			s.increments = s.increments.map((i) => (i.increment_id === event.increment_id ? { ...i, change_id: event.change_id, status: "active" } : i));
			s.budgets = { ...s.budgets, increments_started: s.budgets.increments_started + 1 };
			return s;
		case "increment.status":
			s.increments = s.increments.map((i) => (i.increment_id === event.increment_id ? { ...i, status: event.status, result_note: event.note } : i));
			return recomputeEligibility(s);
...
export function decideProgram(state: ProgramState | null, command: ProgramCommand): ProgramDecision {
	...
	try {
		if (command.type === "program.create") {
			...
			const budgets = { max_increments: 50, increments_started: 0, program_ms: 24 * 3600_000, program_ms_used: 0, ...(command.budgets ?? {}) };
			return { ok: true, events: [{ type: "program.created", ...base, program_id: command.program_id, project_path: command.project_path, objective: command.objective, title: command.title, budgets }] };
		}
		if (!state) throw new DomainError("UNKNOWN_REFERENCE", "program does not exist");
		if (state.closed && command.type !== "milestone.evaluate") throw new DomainError("INVALID_TRANSITION", "program is closed");
		if (command.actor.actor_type === "agent") throw new DomainError("POLICY_DENIED", "an agent cannot write the program");
		switch (command.type) {
			case "trajectory.adopt": {
				const ids = new Set<string>();
				for (const inc of command.increments) {
					if (ids.has(inc.increment_id)) throw new DomainError("PRECONDITION_FAILED", `duplicate increment ${inc.increment_id}`);
					ids.add(inc.increment_id);
					if (!inc.closure_criterion.trim()) throw new DomainError("PRECONDITION_FAILED", `increment ${inc.increment_id} has no closure criterion (RM-005)`);
				}
				for (const inc of command.increments) for (const d of inc.depends_on) if (!ids.has(d)) throw new DomainError("UNKNOWN_REFERENCE", `increment ${inc.increment_id} depends on unknown ${d}`);
				const cycle = findCycle(command.increments);
				if (cycle) throw new DomainError("CYCLE_DETECTED", `dependency cycle: ${cycle.join(" -> ")} (RM-006)`, { nextActions: ["revise_trajectory"] });
				for (const inc of state.increments) {
					if ((inc.status === "accepted" || inc.status === "integrated") && !ids.has(inc.increment_id)) throw new DomainError("PRECONDITION_FAILED", `accepted increment ${inc.increment_id} cannot disappear from the trajectory; mark it abandoned explicitly`);
				}
				const covered = new Set(command.increments.flatMap((i) => i.requirement_ids));
				for (const m of command.milestones) {
					for (const id of m.increment_ids) if (!ids.has(id)) throw new DomainError("UNKNOWN_REFERENCE", `milestone ${m.milestone_id} references unknown increment ${id}`);
					for (const rid of m.global_requirement_ids) if (!covered.has(rid) && !m.global_requirement_ids.includes(rid)) throw new DomainError("PRECONDITION_FAILED", `global requirement ${rid} is neither assigned to an increment nor to a milestone control`);
				}
				return { ok: true, events: [{ type: "trajectory.adopted", ...base, revision: state.trajectory_revision + 1, increments: command.increments, milestones: command.milestones, reason: command.reason }] };
			}
			case "increment.bind": {
				...
				if (inc.status !== "ready") throw new DomainError("PRECONDITION_FAILED", `increment ${inc.increment_id} is ${inc.status}, not ready`);
				if (state.increments.some((i) => i.status === "active")) throw new DomainError("OPERATION_ACTIVE", "P0 runs one producing increment at a time");
				if (state.budgets.increments_started >= state.budgets.max_increments) throw new DomainError("BUDGET_EXHAUSTED", "program increment budget exhausted");
				return { ok: true, events: [{ type: "increment.bound", ...base, increment_id: command.increment_id, change_id: command.change_id }] };
			}
			case "increment.result": {
				const inc = state.increments.find((i) => i.increment_id === command.increment_id);
				if (!inc) throw new DomainError("UNKNOWN_REFERENCE", `unknown increment ${command.increment_id}`);
				return { ok: true, events: [{ type: "increment.status", ...base, increment_id: command.increment_id, status: command.status, note: command.note }] };
			}
			case "milestone.evaluate": {
				...
				if (m.final && evaluation.verdict === "PASS") events.push({ type: "program.closed", ...base, reason: "final milestone passed" });
...
export function recomputeEligibility(state: ProgramState): ProgramState {
	...
	const done = (id: string) => { const s = byId.get(id)?.status; return s === "accepted" || s === "integrated"; };
...
export function dependentsOf(state: ProgramState, incrementId: string): string[] {
	...
	while (changed) {
		...
		for (const inc of state.increments) {
			...
			if (inc.depends_on.includes(incrementId) || inc.depends_on.some((d) => out.has(d))) { out.add(inc.increment_id); changed = true; }
		}
	}
	return [...out];
}
...
export function findCycle(increments: readonly IncrementSpec[]): string[] | null {
	...
	const visit = (id: string): string[] | null => {
		...
		if (st === 1) { const idx = stack.indexOf(id); return [...stack.slice(idx), id]; }
		if (st === 2) return null;
		state.set(id, 1);
		stack.push(id);
		for (const d of adj.get(id) ?? []) { const c = visit(d); if (c) return c; }
		stack.pop();
		state.set(id, 2);
		return null;
	};
	for (const inc of increments) { const c = visit(inc.increment_id); if (c) return c; }
	return null;
}

/** Milestone verdict is recomputed from global obligations; it is never the sum of child statuses (RM-007, RM-008, PRG-05). */
export function evaluateMilestone(state: ProgramState, m: Milestone, globalVerdicts: Record<string, Verdict>, integratedDigest: string | null, at: string): MilestoneEvaluation {
	...
	for (const id of m.increment_ids) {
		...
		if (!inc) { remaining.push(`increment:${id}`); continue; }
	...
	else if (remaining.length > 0 || indeterminate.length > 0) verdict = remaining.length > 0 && indeterminate.length === 0 ? "NOT_RUN" : "INDETERMINATE";
	else verdict = "PASS";
	return { milestone_id: m.milestone_id, verdict, satisfied, remaining, abandoned, indeterminate, evaluated_at: at, integrated_digest: integratedDigest };
}
