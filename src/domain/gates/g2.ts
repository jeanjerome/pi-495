...
export function evaluateG2(state: ChangeState, protocol: Protocol, policy: ActivePolicy): G2Result {
	...
	if (state.environment_digest && protocol.environment_digest !== state.environment_digest) reasons.push("protocol environment digest differs from the current environment");
	...
	for (const o of protocol.obligations) {
		...
		if (o.not_applicable_reason) {
			if (!o.not_applicable_reason.trim()) reasons.push(`requirement ${rid}: empty non-applicability justification (RM-017)`);
			continue;
		}
		if (o.combination === "human_decision" || o.human_interaction) {
			if (o.human_interaction !== "IH-10") reasons.push(`requirement ${rid}: human decision assigned without interaction`);
			continue;
		}
		...
		for (const cid of o.control_ids) {
			...
			if (q.incident !== "INDETERMINATE") reasons.push(`control ${cid}: sensor incident gave ${q.incident}, expected INDETERMINATE`);
			if (q.environment_digest !== protocol.environment_digest) reasons.push(`control ${cid}: qualified in another environment`);
	...
	for (const rid of state.mandatory_requirement_ids) if (!obligated.has(rid)) { uncovered.push(rid); reasons.push(`mandatory requirement ${rid} has no obligation`); }
