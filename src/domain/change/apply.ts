...
export function apply(state: ChangeState | null, event: ChangeEvent): ChangeState {
	...
	switch (event.type) {
		...
		case "artifact.adopted":
			s.adopted = {
				...s.adopted,
				[event.kind]: { kind: event.kind, ref: event.ref, adopted_at: event.at, gate: event.gate },
			};
			return s;
		...
		case "attempt.closed":
			s.attempts = s.attempts.map((a) =>
				a.attempt_id === event.attempt_id ? { ...a, result: event.result, ended_at: event.at } : a,
			);
			return s;
		case "intervention.started":
			...
			if (event.attempt_id)
				s.attempts = s.attempts.map((a) =>
					a.attempt_id === event.attempt_id ? { ...a, interventions: [...a.interventions, event.intervention_id] } : a,
				);
			s.status = "running";
			return s;
		...
		case "budget.consumed":
			s.interventions = s.interventions.map((i) =>
				i.intervention_id === event.intervention_id ? { ...i, counters: addCounters(i.counters, event.counters) } : i,
			);
			return addCountersToAttempt(s, event.intervention_id, event.counters);
		case "budget.extended":
			s.budgets = {
				...s.budgets,
				max_attempts: event.new_max_attempts,
				extensions: [...s.budgets.extensions, { amount: event.amount, decision_id: event.decision_id, at: event.at }],
			};
			return s;
		case "candidate.frozen":
			...
			s.attempts = s.attempts.map((a) =>
				a.attempt_id === event.attempt_id
					? { ...a, candidate: event.candidate, result: "completed", ended_at: event.at }
					: a,
			);
			return s;
		...
		case "evidence.invalidated":
			s.evidence = s.evidence.map((e) =>
				e.evidence_id === event.evidence_id ? { ...e, valid: false, invalid_reason: event.reason } : e,
			);
			return s;
		case "review.recorded":
			s.reviews = [
				...s.reviews,
				{
					...
					blocking_findings: event.blocking_findings,
					valid: true,
					recorded_at: event.at,
				},
			];
		...
		case "decision.recorded":
			...
			s.human_decisions = [
				...s.human_decisions,
				{
					human_decision_id: event.human_decision_id,
					decision_id: event.decision_id,
					interaction: event.interaction,
			...
			if (event.interaction === "IH-10" && event.option_id === "accept")
		...
		case "operation.effect":
			if (s.operation && s.operation.operation_id === event.operation_id)
				s.operation = { ...s.operation, effect_state: event.effect_state };
			return s;
		...
		case "integration.prepared":
			s.integration = {
				...
				destination_before: event.destination_before,
		...
		case "integration.confirmed":
			if (s.integration)
				s.integration = {
					...s.integration,
					destination_after: event.destination_after,
...
function addCountersToAttempt(
	...
	s.budgets = {
		...s.budgets,
		increment_ms_used: s.budgets.increment_ms_used + counters.duration_ms,
		tool_calls_total: s.budgets.tool_calls_total + counters.tool_calls,
	};
	if (intervention?.attempt_id) {
		s.attempts = s.attempts.map((a) =>
			a.attempt_id === intervention.attempt_id ? { ...a, counters: addCounters(a.counters, counters) } : a,
		);
	}
	return s;
}
