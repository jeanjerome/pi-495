import type { ActorRef, ArtifactRef, CandidateRef, GateId, Phase, ExecStatus, Outcome, StopReason, SubjectRef, HumanInteraction, Verdict, InterventionRole } from "../../contracts/v1/common.ts";
...
export type ChangeEvent =
	...
	| (Base & { type: "artifact.revised"; kind: ArtifactKind; ref: ArtifactRef; rollback_phase: Phase; invalidated_gates: GateId[]; reason: string })
	| (Base & { type: "mandate.recorded"; allowed_paths: string[]; integration: "disabled" | "local_branch"; language: "fr" | "en" })
	...
	| (Base & { type: "evidence.recorded"; evidence_id: string; control_id: string; control_version: string; requirement_ids: string[]; subject_digest: string; protocol_revision: number; environment_digest: string; verdict: Verdict; findings_blocking: number })
	...
	| (Base & { type: "review.recorded"; review_id: string; reviewer_role: string; subject_digest: string; conclusion: "approve" | "reject" | "consultative"; blocking_findings: number })
	| (Base & { type: "review.invalidated"; review_id: string; reason: string })
	| (Base & { type: "decision.requested"; decision_id: string; interaction: HumanInteraction; subject: SubjectRef; expires_at: string | null })
	| (Base & { type: "decision.recorded"; human_decision_id: string; decision_id: string; interaction: HumanInteraction; option_id: string | null; subject: SubjectRef; actor_id: string; scope: string | null })
	...
	| (Base & { type: "operation.effect"; operation_id: string; effect_state: "prepared" | "started" | "confirmed" | "failed" | "uncertain" | "reconciled"; detail: string | null })
	| (Base & { type: "operation.closed"; operation_id: string })
	| (Base & { type: "operation.retried"; operation_key: string; count: number })
	| (Base & { type: "integration.prepared"; destination: string; destination_before: string; candidate_digest: string; plan_digest: string; authorized_by: string })
