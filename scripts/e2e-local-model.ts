...
const agent = new PiWorkerAgent({
	config: {
		pi_package_dir: piDir,
		pi_agent_dir: join(homedir(), ".pi", "agent"),
		sandbox_backend: process.platform === "darwin" ? "seatbelt" : "unconfined",
		denied_read_paths: [],
		heartbeat_ms: 5000,
	},
	silence_timeout_ms: 300_000,
});
const mandate: InterventionMandate = {
	...
	prompt: [
		"You work in the current directory, a small JavaScript project.",
		"Task: modify src/greet.js so that greet(name) returns `Hello, ${name}!` (with a trailing exclamation mark) and update test/greet.test.js to expect it.",
		"Use the read, edit, write and bash tools. Run `node --test` with bash to check.",
		'When finished, answer with a short summary followed by a fenced ```json block with exactly these fields: {"summary": string, "changed_paths": string[], "tests_claimed": boolean, "notes": string[]}.',
	].join("\n"),
	system_prompt:
		"You are a careful software engineer working inside an isolated workspace. Only the workspace is writable. Do not attempt to access other directories or the network.",
	context: {
		role: "implement",
		objective: "greet",
...
for await (const event of handle.events) {
	...
	if (event.type === "model_event") console.log(`[${t}s] model ${event.kind} ${event.tokens ?? ""}`);
		...
		console.log(`[${t}s] tool ${event.tool} ${event.is_error ? "error" : "ok"}${event.blocked ? " BLOCKED" : ""}`);
